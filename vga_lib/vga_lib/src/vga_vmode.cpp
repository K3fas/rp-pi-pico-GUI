...
void VgaCfg(const sVgaCfg *cfg, sVmode *vmode)
{
	...
	for (i = 1; i < LAYERS; i++)
	{
		if (cfg->mode[i] != LAYERMODE_BASE)
			prog = LayerMode[cfg->mode[i]].prog;
	...
	if (wfull == 0)
		wfull = w; // use required width as 100% width
	...
	if (cfg->dbly)
		h *= 2;			   // use double lines
	vmode->vmax = v->vmax; // maximal height
	if (h > v->vmax)
		h = v->vmax; // limit height
	if (cfg->dbly)
		h &= ~1; // must be even number if double lines
	...
	if (cfg->dbly)
		h /= 2; // return double lines to single lines
	vmode->height = h;
