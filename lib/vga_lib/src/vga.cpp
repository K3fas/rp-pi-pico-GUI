	...
	if (y0 >= 0)
	{
		...
		for (layer = 1; layer < LAYERS; layer++)
		{
			...
			do
			{
				...
				if (a <= CurLayerProg.maxidle + LAYER_OFFSET)
					break;
...
void VgaDmaInit()
{
	...
	{
		// layer is not active
		if ((layer > 0) && (LayerModeInx[layer] == LAYERMODE_BASE))
			continue;
...
void VgaPioInit()
{
	...
	for (i = VGA_GPIO_FIRST; i <= VGA_GPIO_LAST; i++)
		pio_gpio_init(VGA_PIO, i);

	// negative HSYNC output
	if (!CurVmode.psync)
		gpio_set_outover(VGA_GPIO_SYNC, GPIO_OVERRIDE_INVERT);
...
void ScanlineTypeInit(const sVmode *v)
{
	...
	*d++ = LINE_DARK;
	...
	{
		// vertical sync (VGA 2)
		for (i = v->vsync1; i > 0; i--)
			*d++ = LINE_VSYNC;

		// dark (VGA 33)
		for (i = v->vback1; i > 0; i--)
			*d++ = LINE_DARK;
	...
	else
	{
		...
		if (v->odd)
			for (i = v->vact1; i > 0; i--)
				*d++ = LINE_IMGODD1; // odd lines 1, 3, 5, ... (PAL)
		else
			for (i = v->vact1; i > 0; i--)
				*d++ = LINE_IMGEVEN1; // even lines 0, 2, 4, ... (NTSC)
		...
		for (i = v->vpost2 / 2; i > 0; i--)
			*d++ = LINE_HHSYNC;

		// dark (PAL 18+23, NTSC 11+2)
		for (i = v->vback2; i > 0; i--)
			*d++ = LINE_DARK;

		// image 2nd sub-frame (PAL 240, NTSC 240)
		if (v->odd)
			for (i = v->vact2; i > 0; i--)
				*d++ = LINE_IMGEVEN2; // even lines 0, 2, 4, ... (PAL)
		else
			for (i = v->vact2; i > 0; i--)
				*d++ = LINE_IMGODD2; // odd lines 1, 3, 5, ... (NTSC)

		// dark (PAL 24, NTSC 1)
		for (i = v->vfront2; i > 0; i--)
			*d++ = LINE_DARK;

		// half sync (PAL 3, NTSC 3)
		for (i = v->vpre2 / 2; i > 0; i--)
			*d++ = LINE_HHSYNC;
	}
}
...
void VgaInit(const sVmode *vmode)
{
	...
	for (i = 1; i < LAYERS; i++)
	{
		...
		if (LayerModeInx[i] != LAYERMODE_BASE)
			LayerMask |= (1 << i);
	}
...
{
	if (vmode == NULL)
		vmode = (const sVmode *)1;
	__dmb();
	VgaVmodeReq = vmode;
	while (VgaVmodeReq != NULL)
	{
		__dmb();
	}
}
